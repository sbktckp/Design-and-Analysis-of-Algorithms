...
Open the repo in a Codespace (Code -> Codespaces -> Create codespace on main), then:
...
Run the binaries from inside `Day 1/` so the relative `data/` paths resolve.
Build a single program instead: `make bin/1.2_prefix_sum`. Clean up: `make clean`.
